...
public class Persistencia_Cliente {
    ...
    public void inserirCliente(Model_Cliente objModel_Cliente) {
        try {
            objCall = objConectar.getConexao().prepareCall("CALL PROC_INSERIR(?)");
            ...
            objCall.setString(5, objModel_Cliente.getA01_credito());
            objCall.execute();
    ...
    public void alterarCliente(Model_Cliente objModel_Cliente) {
        try {
            objCall = objConectar.getConexao().prepareCall("CALL PROC_ALTERAR(?, ?)");
            ...
            objCall.setString(6, objModel_Cliente.getA01_credito());
            objCall.execute();
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public void deletarCliente(int a01_codigo) {
        try {
            objCall = objConectar.getConexao().prepareCall("CALL PROC_DELETAR(?)");
